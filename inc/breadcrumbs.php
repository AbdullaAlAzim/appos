 ...
 */
function appos_breadcrumbs($class = '') {

    // Settings
    $separator   = '&gt;';
    $breadcrums_id   = 'breadcrumbs';
    $breadcrums_class    = 'breadcrumb-ul ' . $class;
    $home_title  = esc_html__('Home', 'appos');
    ...
    if ( !is_front_page() ) {
        ...
        if ( is_author() ) {
            ...
            // Paginated archives
            echo '<li class="item-current item-current-' . get_query_var('paged') . '"><span class="bread-current bread-current-' . get_query_var('paged') . '" title="Page ' . get_query_var('paged') . '">'.esc_html__('Page','appos') . ' ' . get_query_var('paged') . '</span></li>';
